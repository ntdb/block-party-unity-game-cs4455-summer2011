...
function ShowGUI() {
	...
	if (Network.peerType == NetworkPeerType.Disconnected)
	{
		...
		remoteIP = GUI.TextField(new Rect(120,65,100,20), remoteIP);
		remotePort = parseInt(GUI.TextField(new Rect(230,65,50,20),remotePort.ToString()));
		
		if (GUI.Button (new Rect(10,60,90,30), "Connect"))
		{
			Network.useNat = false;
	...
	else
	{
		...
		if(isServer)
		{
			GUI.Label(new Rect(Screen.width-200, 10, 200, 20), "Server IP: " + Network.player.ipAddress.ToString() + ":" + Network.player.port.ToString());
		}
		else if (remoteIP != "127.0.0.1")
		{
			GUI.Label(new Rect(Screen.width-200, 10, 200, 20), "Server IP: " + remoteIP.ToString() + ":" + remotePort.ToString());
		}
	}
