...
var wingsPrefab : Transform;
var hasGlidePowerUp : boolean = false;
var glideFallSpeed : float = 2;
private var originalMaxFallSpeed : int = 0;
private var wingsScript : WingsController;

var skatesPrefab : Transform;
var rocketAcceleration : float = 100.0;
var maxRocketSpeed : float = 30.0;
private var skatesScript : JetSkates;
private var hasRocketSkates : boolean = false;
private var originalMaxSpeed : float;
...
function GetGlidePowerUp(){
	...
	var wings : Transform = Network.Instantiate(wingsPrefab, Vector3(transform.position.x, transform.position.y - 0.5, transform.position.z), transform.rotation, 2);
...
function GetRocketSkatesPowerUp(){
	var rocketSkates : Transform = Network.Instantiate(skatesPrefab, transform.position, transform.rotation, 2);
	...
	transform.position.y = transform.position.y + 0.35;
	hasRocketSkates = true;
	originalMaxSpeed = movement.maxForwardSpeed;
	movement.maxForwardSpeed = maxRocketSpeed;
	movement.maxSidewaysSpeed = maxRocketSpeed;
	movement.maxBackwardsSpeed = maxRocketSpeed;
}
...
private function ApplyGravityAndJumping (velocity : Vector3) {
	...
	if (grounded)
		...
		// custom: check for glide power up
		if(hasGlidePowerUp && inputJump){
			wingsScript.ActivateWings();
			movement.maxFallSpeed = glideFallSpeed;
		} else if(hasGlidePowerUp){
			wingsScript.DeactivateWings();
			movement.maxFallSpeed = originalMaxFallSpeed;
		}
...
function MaxSpeedInDirection (desiredMovementDirection : Vector3) : float {
	var actionButtonPressed : boolean = Input.GetButton("Action");
	if(hasRocketSkates){
		if(actionButtonPressed){
			if(desiredMovementDirection == Vector3.zero){
				skatesScript.TurnOffJets();
				return 0;
			}
			else
				skatesScript.TurnOnJets();
		}
		else
			skatesScript.TurnOffJets();
	}
	...
	if (desiredMovementDirection == Vector3.zero)
		...
		if(hasRocketSkates){
			if(actionButtonPressed)
				movementSpeed = movement.maxForwardSpeed;
			else
				movementSpeed = originalMaxSpeed;
		} else
			movementSpeed = movement.maxForwardSpeed;
		//var zAxisEllipseMultiplier : float = (desiredMovementDirection.z > 0 ? movementSpeed : movementSpeed) / movementSpeed;
		var temp : Vector3 = new Vector3(desiredMovementDirection.x, 0, desiredMovementDirection.z /*/ AxisEllipseMultiplier*/).normalized;
		var length : float = new Vector3(temp.x, 0, temp.z /* * zAxisEllipseMultiplier*/).magnitude * movementSpeed;
		return length;
	}
}
